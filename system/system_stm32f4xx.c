...
#if defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F446xx) || defined(STM32F469_479xx)
  uint32_t SystemCoreClock = 168000000;//180000000
#endif /* STM32F427_437x || STM32F429_439xx || STM32F446xx || STM32F469_479xx */
...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* HCLK = SYSCLK / 1*/
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;//HCLK

#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) ||  defined(STM32F412xG) || defined(STM32F446xx) || defined(STM32F469_479xx)    
    /* PCLK2 = HCLK / 2*/
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;//APB高速
    
    /* PCLK1 = HCLK / 4*/
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV4;//APB低速
    ...
    //PLLSRC PLLSRL为HSE
    //f(VCO 时钟) = f(PLL 时钟输入) × (PLLN / PLLM)           输入24MHz VCO=336MHz
    //f(PLL 常规时钟输出) = f(VCO 时钟) / PLLP                PLL=168MHz
    //f(USB OTG FS, SDIO, RNG 时钟输出) = f(VCO 时钟) / PLLQ  USB SDIO=48MHz
    ...
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));//SW清零 选择PLLCLK作为系统时钟
...
#elif defined(STM32F410xx) || defined(STM32F411xE)
#if defined(USE_HSE_BYPASS) //晶振旁路
/******************************************************************************/
/*            PLL (clocked by HSE) used as System clock source                */
/******************************************************************************/
  __IO uint32_t StartUpCounter = 0, HSEStatus = 0;
...
/*
 ...
 | PD10 <-> FMC_D15 | PE9  <-> FMC_D6   | PF12 <-> FMC_A6 | PG7 <-> FMC_NE1 | 
 | PD11 <-> FMC_A16 | PE10 <-> FMC_D7   | PF13 <-> FMC_A7 | PG9 <-> FMC_NE2 | 
 | PD12 <-> FMC_A17 | PE11 <-> FMC_D8   | PF14 <-> FMC_A8 | PG10<-> FMC_NE3 | 
 | PD13 <-> FMC_A18 | PE12 <-> FMC_D9   | PF15 <-> FMC_A9 | PG12<-> FMC_NE4 | 
...
*/
  ...
  /* Configure PGx pins speed to 50 MHz */ 
  GPIOG->OSPEEDR = 0x00200aaa;
  #elif (SRAM_BANK_SELECT == 0x00000006)
  /* Connect PGx pins to FMC Alternate function */
  GPIOG->AFR[0]  = 0x00cccccc;
  GPIOG->AFR[1]  = 0x000c0000;
  /* Configure PGx pins in Alternate function mode */ 
  GPIOG->MODER   = 0x02000aaa;
  /* Configure PGx pins speed to 50 MHz */ 
  GPIOG->OSPEEDR = 0x02000aaa;
  #endif
  /* Configure PGx pins Output type to push-pull */  
  GPIOG->OTYPER  = 0x00000000;
  /* No pull-up, pull-down for PGx pins */ 
...
/*-- FMC Configuration ------------------------------------------------------*/
...
#if defined(STM32F40_41xxx)
  /* Configure and enable Bank1_SRAM2 */
  //FSMC_DataAddressMux_Disable;           //地址/数据复用使能位 (Address/data multiplexing enable bit) 不复用数据地址
  //FSMC_MemoryType_SRAM;                  //存储器类型 (Memory type) SRAM
  //FSMC_MemoryDataWidth_16b;              //存储器数据总线宽度 (Memory databus width) 16bit
  //FSMC_BurstAccessMode_Disable;          //同步模式 突发使能位 (Burst enable bit)
  //FSMC_WaitSignalPolarity_Low;           //等待信号极性位 (Wait signal polarity bit) 仅当15位ASYNCWAIT置1才有效
  //FSMC_WrapMode_Disable;                 //同步模式 环回突发模式支持 (Wrapped burst mode support)
  ...
  FSMC_Bank1->BTCR[SRAM_BANK_SELECT]  = 0x00001011;
  FSMC_Bank1->BTCR[SRAM_BANK_SELECT + 1]  = (SRAM_ADDRSET | (ADDRHLD << 4) | (DATASET << 8) | (BUSTURN << 16));
  FSMC_Bank1E->BWTR[SRAM_BANK_SELECT] = 0x0fffffff;
#endif  /* STM32F40_41xxx */
